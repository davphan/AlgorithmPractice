class MaxHeap(list):
    ...
    def __init__(self, arr:list):
        """Max heap data structure. Supports peek, insert, and delete functions.

        Instantiation Time complexity: O(nlog(n))
        Instantiation Space complexity: O(n)

        Args:
            arr (list): List of values to insert into the max heap.
        """
        super().__init__()
        for num in arr:
            self.insert(num)
    ...
    def peek(self):
        """Return the largest value of the heap.
        Time complexity: O(1)

        Returns:
            Any: Largest value of the heap.
        """
        return self[0]
    ...
    def delete(self):
        """Deletes the largest value from the top of the heap and returns the value
        ...
        # Decide which direction to "bubble down", the largest child
        nextShift = left if left >= len(self) or right >= len(self) or self[left] > self[right] else right

        # Check if current node is smaller than its largest child, and swap
        # elements, "bubble down"
        while nextShift < len(self) and self[i] < self[nextShift]:

            # Swap current element with largest child
